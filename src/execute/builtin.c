...
int	builtin_cmd(t_tree *node, t_envp *env)
{
	...
	args = each_args(node, env, 0);

	if (ft_strncmp(node->value, "env", ft_strlen("env")) == 0)
		ft_env(args, env->u_envp);
	if (ft_strncmp(node->value, "exit", ft_strlen("exit")) == 0)
		ft_exit_call();
	if (ft_strncmp(node->value, "pwd", ft_strlen("pwd")) == 0)
		ft_pwd();
	if (ft_strncmp(node->value, "export", ft_strlen("export")) == 0)
		export(node, args, env->u_envp);
	if (ft_strncmp(node->value, "unset", ft_strlen("unset")) == 0)
		unset(args, env->u_envp);
	if (ft_strncmp(node->value, "cd", ft_strlen("cd")) == 0)
		ft_cd(node, args, env->u_envp);
	if (ft_strncmp(node->value, "echo", ft_strlen("echo")) == 0)
		ft_echo(args, env->u_envp);
	return (1);
}
